...
cred = credentials.Certificate(
    os.path.join(settings.BASE_DIR, "google-services.json")
)
    ...
    def save(self):
        ...
        if self.user.display_name:
            user, is_new = User.objects.get_or_create(
                username=self.user.uid, first_name=self.user.display_name
            )
        else:
            user, is_new = User.objects.get_or_create(
        ...
        if is_new:
            ...
            models.FirebaseToken.objects.filter(user=user).update(
                fcm_token=fcm_token
            )
